...
def squeeze(data, num_filter, name, kernel=(1,1), stride=(1,1), pad=(0, 0), act_type="relu", mirror_attr={}):
    squeeze_1x1=mx.symbol.Convolution(data=data, num_filter=num_filter, kernel=kernel, stride=stride, pad=pad, name=name)
...
def Fire_module(data, num_filter_squeeze, num_filter_fire,fire_num,kernel_sequeeze=(1,1),kernel_1x1=(1,1),
    ...
    expand1x1=mx.symbol.Convolution(data=squeeze_1x1, num_filter=num_filter_fire, kernel=kernel_1x1, stride=stride_1x1, pad=pad_1x1, name="fire%d_expand1x1"%fire_num)
    relu_expand1x1=mx.symbol.Activation(data = expand1x1, act_type=act_type, attr=mirror_attr, name="fire%d_expand1x1_relu"%fire_num)
    
    expand3x3=mx.symbol.Convolution(data=squeeze_1x1, num_filter=num_filter_fire, kernel=kernel_3x3, stride=stride_3x3, pad=pad_3x3, name="fire%d_expand3x3"%fire_num)
    relu_expand3x3=mx.symbol.Activation(data = expand3x3, act_type=act_type, attr=mirror_attr, name="fire%d_expand3x3_relu"%fire_num)
...
def SqueezeNetV1_1(data, num_classes):
    ...
    drop9=mx.sym.Dropout(data=fire9, p=0.5)
    conv10=mx.symbol.Convolution(data=drop9, num_filter=num_classes, kernel=(1,1), stride=(1,1), name="conv10")
    relu_conv10=mx.symbol.Activation(data = conv10, act_type="relu", attr={})
